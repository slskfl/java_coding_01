...
public final class Strings {
	...
	private static void permuteAndPrintStream(String prefix, String str) {
		...
		if (n == 0) {
			...
			IntStream.range(0, n).parallel().forEach(i -> permuteAndPrintStream(prefix + str.charAt(i), str.substring(i + 1, n) + str.substring(0, i)));
		}
	}
	...
	public static Stream<String> permuteAndReturnStream(String str) {
		...
		return IntStream.range(0, str.length()).parallel().boxed().flatMap(i -> permuteAndReturnStream(str.substring(0, i) + str.substring(i + 1)).map(c -> str.charAt(i) + c));
	}
}
